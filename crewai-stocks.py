...
# Função para buscar o preço das ações usando a API do Yahoo Finance
def fetch_stock_price(ticket):
    stock = yf.download(ticket, start="2023-08-08", end="2024-08-08")
    return stock

# Criando uma ferramenta para buscar preços de ações
...
# Criando um agente para análise de preços de ações
...
# Definindo uma tarefa para analisar o histórico de preços de ações
getStockPrice = Task(
...
# Criando uma ferramenta de busca de notícias usando DuckDuckGo
search_tool = DuckDuckGoSearchResults(backend='news', num_results=10)

# Criando um agente para análise de notícias de mercado
newsAnalyst = Agent(
...
# Definindo uma tarefa para obter notícias de mercado
get_news = Task(
...
# Criando um agente para escrever análises de ações
...
# Definindo uma tarefa para escrever a análise de ações
writeAnalyses = Task(
...
# Criando uma equipe de agentes para executar as tarefas
crew = Crew(
